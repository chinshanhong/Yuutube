...
public class Category {
    ...
    //Method to display categories available
    public static void displayCategories(){
        ...
        try{
            ...
            String sql = "SELECT DISTINCT videoCategory from video";
            ...
            while(rs.next()){
                ...
                categoryNameList.add(rs.getString("videoCategory"));
                System.out.printf("%d. %s", count, category);
                System.out.println("");
                count += 1;
                
            }
            //Prompt the user the select the video category they want to watch
            while(true){
                System.out.print("Please select[1-" + categoryNameList.size() + "]: ");
                int user_input = scan.nextInt();
                if(user_input >= 1 && user_input <= categoryNameList.size()){
                    
                    //After user made a choice, call displayCategoryVideo() method
    ...
    //Method to display video according to categories
    public static void displayCategoryVideo(String categoryName){
        ...
        try{
            Class.forName("com.mysql.cj.jdbc.Driver");
            conn = DriverManager.getConnection(DB_URL, USER, PASS);
            String sql = "SELECT* FROM video WHERE videoCategory=?";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, categoryName);
            ResultSet rs = stmt.executeQuery();
            
            //Display 5 videos acoording to categories
            int count = 1;
            while(rs.next()){
                ...
                String uploaderName = rs.getString("user_id");
                String category = rs.getString("videoCategory");
                f2.format("%d%60s%20d%25s%25s%25s", count, title, view, 
                        duration,  uploaderName, category);
                System.out.println(f2);
                System.out.println("");
                count += 1;
                
            }
            /*After displaying videos according to category, call the 
            categoryVideoOperation() method with videoIdList as parameter */
    ...
    /*Method to allow the user to select the video they want to watch according to
    categories*/
    public static void categoryVideoOperation(ArrayList<Integer> videoIdList)throws IOException{
        ...
        while(true){
            ...
            int user_input = scan.nextInt();
            if(user_input > 0 && user_input <= videoIdList.size()){
